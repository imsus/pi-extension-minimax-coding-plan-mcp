 ...
 */
function removeApiKeyFromAuthFile() {
    const homedirPath = homedir();
    const authFilePath = join(homedirPath, ".pi", "agent", "auth.json");
    if (!existsSync(authFilePath)) {
        return;
  ...
  https://platform.minimax.io/subscribe/coding-plan
        `.trim();
            ...
            if (args?.includes("--clear")) {
                const confirmClear = await ctx.ui.confirm("Clear MiniMax Configuration", "This will remove your API key from ~/.pi/agent/auth.json");
                if (confirmClear) {
            ...
            if (keyMatch) {
                ...
                const confirmSave = await ctx.ui.confirm("Save MiniMax API Key?", `This will save to ~/.pi/agent/auth.json`);
                if (confirmSave) {
...
Your API key will be saved to ~/.pi/agent/auth.json
      `.trim();
            ...
            if (apiKey && apiKey.trim()) {
                const confirmSave = await ctx.ui.confirm("Save MiniMax API Key?", "Save this API key to ~/.pi/agent/auth.json?");
                if (confirmSave) {
    ...
    pi.registerCommand("minimax-status", {
        ...
        handler: async (_args, ctx) => {
            if (config.configured) {
                ...
                ctx.ui.notify("✗ MiniMax MCP not configured\n\nUse /minimax-configure to set up your API key", "warning");
            }
        },
    });
...
- Be specific: "TypeScript 5.4 generics" instead of "TypeScript help"`,
        ...
        async execute(toolCallId, params, onUpdate, ctx, signal) {
            ...
            if (!config.configured) {
                return createErrorResult("MiniMax API key not configured", "Use /minimax-configure to set your API key, or set MINIMAX_API_KEY environment variable");
            }
            if (!params.query || params.query.trim().length < 2) {
                return createErrorResult("Invalid query", "Query must be at least 2 characters long");
            }
            // Show progress
            onUpdate?.({
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 401 || response.status === 403) {
                        config.configured = false;
                        return createErrorResult("Authentication failed", "Invalid API key. Check your API key and API host. Global: https://api.minimax.io, Mainland China: https://api.minimaxi.com");
                    }
                    return createErrorResult(`API error (${response.status})`, errorText || "Unknown error occurred");
                }
                ...
                if (baseResp.status_code !== 0) {
                    switch (baseResp.status_code) {
                        case 1004:
                            return createErrorResult("Authentication error", `${baseResp.status_msg}. Check your API key and API host. Trace-Id: ${response.headers.get("Trace-Id")}`);
                        case 2038:
                            return createErrorResult("Verification required", `${baseResp.status_msg}. Complete real-name verification at https://platform.minimaxi.com/user-center/basic-information. Trace-Id: ${response.headers.get("Trace-Id")}`);
                        default:
                            return createErrorResult(`API error (${baseResp.status_code})`, `${baseResp.status_msg}. Trace-Id: ${response.headers.get("Trace-Id")}`);
                    }
                }
                // Format the results
...
- Identify UI components or code in screenshots`,
        ...
        async execute(toolCallId, params, onUpdate, ctx, signal) {
            ...
            if (!params.prompt || !params.image_url) {
                return createErrorResult("Missing parameters", "Both 'prompt' and 'image_url' are required");
            }
            ...
            if (isComplexTask) {
                const confirmAnalyze = await ctx.ui.confirm("Analyze Image?", `This analysis may take time. Continue with: "${params.prompt.slice(0, 50)}..."?`);
                if (!confirmAnalyze) {
                    return {
                        content: [{ type: "text", text: "Analysis cancelled" }],
                        details: { status: "cancelled" },
                    };
                }
            }
            onUpdate?.({
                content: [{ type: "text", text: `🖼 Converting image to base64...` }],
            ...
            try {
                ...
                if (!content) {
                    return createErrorResult("No content returned", "The VLM API didn't return any analysis content");
                }
                return {
...
function formatSearchResults(result) {
    ...
    if (result.related_searches && Array.isArray(result.related_searches) && result.related_searches.length > 0) {
        ...
        result.related_searches.forEach((suggestion, index) => {
